...
const Navbar = () => {
  ...
  return (
    <header className={`fixed top-0 w-full z-50 shadow-2xl ${darkMode ? 'bg-[var(--primary-darkbackground)] text-[var(--primary-foreground)]' : 'bg-[var(--primary-foreground)] text-[var(--primary-darkbackground)]'}`}>
      <nav className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
        ...
        <ul className="hidden md:flex flex-1 justify-center space-x-8 font-medium items-center">
          {navLinks.map(link => (
            <DarkModeWrapper
              ...
              key={link.name}
              darkMode={darkMode}
              className="group relative px-3 py-1 rounded-xl overflow-hidden transition-all duration-300"
            >
              <NavLink
                to={link.href}
                className={({ isActive }) => `
                  ...
                  ${isActive
                    ...
                    : `${darkMode ? "text-gray-200" : "text-gray-700"} hover:scale-105 group-hover:text-[var(--color-secondary)] after:scale-x-0`}
              ...
              <span className={`
                absolute inset-0 z-0 rounded-xl blur-md
                bg-gradient-to-br from-[var(--color-secondary)]/20 via-transparent to-[var(--color-secondary)]/20
                transition-all duration-500
                ${link.href === location.pathname ? "opacity-100 scale-100" : "opacity-0 group-hover:opacity-100 group-hover:scale-105"}
              `}></span>
            </DarkModeWrapper>
          ))}
        </ul>
        ...
        <ul className="hidden md:flex space-x-4 items-center">
          ...
          >
            <Link to="/cart">
              <ShoppingCart className={`${darkMode ? "text-white" : "text-black"} w-5 h-5`} />
      ...
      {menuOpen && (
        <ul className={`md:hidden px-4 pb-4 pt-2 space-y-4 text-center ${darkMode ? 'bg-[var(--primary-darkbackground)] text-white' : 'bg-white text-black'}`}>
          {navLinks.map(link => (
            <li key={link.name}>
              <NavLink
                to={link.href}
